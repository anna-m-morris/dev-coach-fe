  ...
  async componentDidMount() {
    ...
    this.my_channel.bind('client-initiate-signaling', data => {
      ...
      this.initiator_channel.bind('pusher:subscription_error', () => {
        console.log(
          `error subscribing to signaling user ${data.username}`,
        );
      });

      this.initiator_channel.bind(
    ...
    navigator.getUserMedia(
      ...
      stream => {
        ...
        if (!this.has_own_stream) {
          // if the user hasn't already added their own video stream
        ...
        console.log(
          `${this.username} is connecting to remote peer...`,
        );
  ...
  _connectToPeer = (username, stream = false) => {
    ...
    p.on('signal', data => {
      if (this.is_initiator && data) {
        // initiator (User A) receives their peer data
        // (4) user A receives signal
        this.signal = JSON.stringify(data);
      } else {
        // non-initiator (User B) receives peer data from User A
